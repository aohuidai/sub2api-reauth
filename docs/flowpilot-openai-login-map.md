...
| 3. 填写密码 | `getLoginPasswordInput`、`step6LoginFromPasswordPage` | `getLoginPasswordInput`、`step6LoginFromPasswordPage` | 找到 `input[type=password]` 后填写。为满足本项目的学习需求，OpenAI 密码会写入本机 `chrome.storage.local`，关闭 Popup 后仍可恢复。 |
...
| 8. OAuth 授权继续 | `getOAuthConsentForm`、`getPrimaryContinueButton`、`isOAuthConsentPage`、`step8_findAndClick`、`step8_triggerContinue` | 同名函数 | 先定位授权页，再用表单 `requestSubmit` 或原生 click 确认。Popup 会先启动第 9 步监听。 |
...
1. [popup/popup.js](../popup/popup.js) 先查询分组。第 0 步发送 `OPEN_FIRST_OPENAI_REAUTH`，并把首个候选账号和当前 SUB2API 连接信息交给后台。Popup 关闭时会把查询结果、当前步骤和状态提示保存在 `chrome.storage.session`。
...
如果没有已登录的 QQ 邮箱标签，扩展会先打开 `https://wx.mail.qq.com/` 并停在 QQ 登录页。你完成 QQ 登录后，再打开 Popup 点击第 5 步即可。QQ 密码不会写进 Popup、`chrome.storage.local` 或 Git。
